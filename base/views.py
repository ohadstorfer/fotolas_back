...
def create_images_and_waves(request):
    ...
    try:
        ...
        return Response({'error': 'Session album does not exist'}, status=status.HTTP_404_NOT_FOUND)
    ...
    try:
        ...
        for exif_date in exif_dates:
            if exif_date and exif_date != "null":
                try:
                    parsed_date = datetime.strptime(exif_date, "%Y:%m:%d %H:%M:%S")
                except ValueError:
                    parsed_date = None
            else:
                parsed_date = None
        ...
        # Sort URLs by EXIF date, keeping None dates at the end
        sorted_data = sorted(
            zip(original_urls, watermarked_urls, parsed_dates),
            key=lambda x: (x[2] is None, x[2])
        )

        for original_url, watermarked_url, datetime_original in sorted_data:
            if datetime_original:
                if not current_wave or (datetime_original - previous_datetime) > timedelta(seconds=TIME_GAP_THRESHOLD):
                    current_wave = create_wave(session_album_id, watermarked_url)
                previous_datetime = datetime_original
        ...
        # Create images with waves
        if images_and_waves:
            create_images(images_and_waves)

        # Handle images without EXIF dates
        for original_url, watermarked_url in no_exif_images:
            wave = create_wave(session_album_id, watermarked_url)
            create_images([(original_url, watermarked_url, wave)])

        # Update session album status
        session_album.dividedToWaves = bool(images_and_waves or no_exif_images)
        session_album.active = True
        session_album.set_expiration_date()
        session_album.save()
        ...
        print(f"Error processing images and waves: {e}")
        return Response({'error': 'An error occurred while processing images and waves'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


def create_wave(session_album_id, cover_image_url):
    return Wave.objects.create(session_album_id=session_album_id, cover_image=cover_image_url)


def create_images(images_and_waves):
    images_to_create = [
        Img(photo=original_url, WatermarkedPhoto=watermarked_url, wave=wave, SessionAlbum_id=wave.session_album_id)
        for original_url, watermarked_url, wave in images_and_waves
    ]
    Img.objects.bulk_create(images_to_create)
